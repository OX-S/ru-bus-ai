...
flowchart LR
    subgraph ingest[Ingestion]
        nightly[Static GTFS nightly_refresh.py]
        rt[Realtime gtfs_rt_ingestor.py]
    end
    subgraph storage[Storage + Indexes]
        duckdb[(DuckDB Snapshot)]
        postgres[(Postgres GTFS schema)]
        redis[(Redis GTFS-rt cache)]
        faissSemantic[(semantic.faiss)]
        faissRoutes[(routes.faiss)]
    end
    subgraph api[FastAPI backend]
        cacheSvc[transit_cache]
        semanticSvc[semantic_search]
        routerSvc[llm_router]
    end
    subgraph ui[UI]
        react[Vite/React widgets]
    end

    nightly --> duckdb --> faissRoutes
    nightly --> postgres --> faissSemantic
    rt --> redis --> cacheSvc
    postgres --> semanticSvc
    faissSemantic --> semanticSvc
    duckdb --> faissRoutes
    faissRoutes --> routerSvc
    semanticSvc --> routerSvc
    cacheSvc --> api
    routerSvc --> api
    api --> react
```
